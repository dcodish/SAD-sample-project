-- ============================================================
-- SAD_0 — סקריפט יצירת בסיס הנתונים לפרויקט הדוגמה
-- ============================================================
--
-- הקורס עובד מול Azure SQL Database (ראו PREREQS.md חלק C).
-- ב-Azure בסיס הנתונים כבר קיים — נוצר דרך הפורטל — ולכן אין כאן
-- CREATE DATABASE ואין USE:
--
--   * Azure SQL אינו תומך בפקודת USE. כל חיבור מתבצע ישירות
--     לבסיס נתונים אחד ספציפי.
--   * CREATE DATABASE ב-Azure מתבצע דרך הפורטל, לא מתוך סקריפט.
--
-- איך להריץ:
--   * ב-SSMS  — התחברו לבסיס הנתונים של הקבוצה שלכם ב-Azure והריצו את הקובץ.
--   * דרך Claude Code (MCP) — בקשו מ-Claude להריץ את הקובץ. שימו לב:
--     ה-MCP אינו מכיר את המפריד GO (הוא פקודה של SSMS, לא של T-SQL),
--     ולכן Claude צריך לפצל את הקובץ לפי GO ולשלוח כל בלוק בנפרד.
--
-- במסלול המקומי (SQL Server Express) בטלו את ההערה מארבע השורות הבאות,
-- כדי שהסקריפט ייצור את בסיס הנתונים בעצמו:
