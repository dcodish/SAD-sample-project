...
- את ניתוח חלק א׳ + חלק ב׳ — **PDF שאפשר לסמן בו טקסט**, ורצוי גם קובץ ה-Word המקורי
- מחשב נייד עם כל דרישות הקדם מותקנות
- חשבון GitHub (כל חבר קבוצה צריך אחד)
- את תיקיית הפרויקט של הקבוצה, מוכנה
- ☁️ **טלפון עם שיתוף אינטרנט (hotspot)** — רשת האוניברסיטה חוסמת את Azure SQL
...
☁️ **ובמסלול Azure — בדיקה רביעית, מהרשת שבה תשבו בשיעור:**

```
Test-NetConnection <servername>.database.windows.net -Port 1433
```

`TcpTestSucceeded : False` = תצטרכו hotspot מהטלפון. בדקו את זה **לפני** השיעור.
...
⚠️ **☁️ רשת האוניברסיטה חוסמת את פורט 1433 — ו-Azure SQL עובד רק דרכו.**
ההתקנה תצליח וכל שאילתה תיתקע. **החלפת שרת ה-MCP לא תעזור.**
הפתרון: **hotspot מהטלפון** במחשב שמריץ את Claude.

💾 **commit:** `.gitignore` וה-commit הראשון של הפרויקט.

# עבודה בקבוצה — מי עושה מה

**בשיעור: חבר אחד מקליד, השאר בודקים.**

- המכסה היא של **החשבון**, לא של המחשב — ארבעה במקביל מרוקנים אותה פי ארבע
- ארבעה שדוחפים לאותו ענף באותו זמן = קונפליקטים על כל קובץ
- **הבדיקה היא העבודה האמיתית.** Claude מהיר ולא זהיר — מי שקורא את הפלט ומשווה ל-PDF תורם יותר ממי שמקליד

**בסוף השיעור:** המקליד עושה push, כל השאר עושים pull פעם אחת. לכולם אותו פרויקט.

**חבר שלא הצליח למשוך בשיעור — יגלה את זה בבית, לבד.** אל תלכו בלי זה.

# עבודה בקבוצה — לאורך הסמסטר

מכאן מחלקים לפי **קבצים**, כדי ששניים לא יערכו את אותו קובץ:

| מי | מה |
|---|---|
| חבר א׳ | חילוץ המסמכים, `CLAUDE.md` |
| חבר ב׳ | סכמה ו-Stored Procedures |
| ביחד | הישות הראשונה + מסך הכניסה |
| לפי ישות | מסכי ה-CRUD — מסך לכל אחד |

שניים חייבים לגעת באותו קובץ? תאמו לפי **זמן**: אחד מסיים ודוחף לפני שהשני מתחיל.

# ‏git — שלוש פעולות, ושלא יתבלבלו

יש **עותק על המחשב שלכם** ו**עותק משותף ב-GitHub**.

| | מה עושה | לאן | מתי |
|---|---|---|---|
| **commit** | שומר צילום מצב | **מקומי בלבד** | בסוף כל שלב |
| **push** | דוחף ל-GitHub | מקומי ← GitHub | מיד אחרי commit |
| **pull** | מושך מהקבוצה | GitHub ← מקומי | **לפני** שמתחילים |

**commit לבדו לא משתף כלום.** commit = "שמור". push = "שלח".

**`rejected — fetch first` זה לא כישלון** — מישהו דחף לפניכם. pull, ואז push שוב.

# ‏git — מה מדביקים ומתי

**בתחילת כל ישיבת עבודה, לפני הכול:**

> Before we start: pull the latest from our group repo with `git pull origin main`, then show me `git status` and the last few commits.

**בסוף כל שלב** — commit ו-push ב-Prompt אחד (💾 בשקפים מסמן איפה):

> Commit the work from this phase with the commit message "Phase N: ...", then push with `git push origin main`. If the push is rejected, pull first, help me resolve any conflict, and push again.

הנוסח המלא לכל שלב — ב-`PROMPTS_CHEATSHEET`, סעיף git. **אתם לא מקלידים git בטרמינל.**

**לא עולים ל-git:** `.mcp.json`, `app.config`, `cloned/`, `bin/`

# שלב 2 — קודם בודקים את ה-PDF

**שלוש דקות שחוסכות שעה.** קבוצה אחת בשיעור הקודם בילתה כאן **שעה וחצי**.

הסיבה: ל-PDF שלהם לא הייתה **שכבת טקסט** — Claude נאלץ להסתכל על כל עמוד כתמונה.

```
uv run --with pymupdf cloned/scripts/check_pdfs.py docs
```

`NO TEXT LAYER` ← ייצאו מחדש מה-Word (File ← Save As ← PDF), או תנו ל-Claude את ה-`.docx`.

**בדיקה ידנית, 10 שניות:** אי אפשר לסמן טקסט ב-PDF עם העכבר? זה PDF של תמונות.

**הדיאגרמות הן חריג מכוון** — הן תמונות, וזה בסדר. Claude יפתח רק את העמודים שלהן.
