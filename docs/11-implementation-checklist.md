...
עבדו לפי הסדר — כל דרג בונה על הקודם.

> **הסדר כאן הוא לפי תלות, לא לפי שלבי השיעור.** ה"דרגים" הם בדיוק סדר הטעינה של
> `initLists()`: ישויות בסיס תחילה, אחריהן ישויות עם מפתחות זרים, ומחלקות קישור אחרונות.
> ‏"שלב N" מופיע רק ב-[`LESSON_STEPS`](../LESSON_STEPS.md) ומתייחס לשלבי השיעור (0–11).

> **דוגמאות מהפרויקט:** בכל דרג מצוין הקובץ הרלוונטי מפרויקט הדוגמה. פתחו אותו וראו איך זה נראה בפועל.

---

## לפני שמתחילים: מניות (Enumerations) — enum ב-C# ואילוץ CHECK ב-DB

לפני שמתחילים לבנות ישויות, בדקו ב-Class Diagram אם יש שדות עם **ערכים קבועים** (כמו סטטוס, סוג, תפקיד, קטגוריה).

> **דוגמה בפרויקט:** `Title.cs` — enum לתפקידי עובדים; העמודה `Workers.workerTitle` עם אילוץ `CK_WORKER_TITLE` ב-`scripts/create_database.sql`

> **לא יוצרים טבלת Lookup לערכים קבועים.** אילוץ `CHECK` על העמודה אוכף בדיוק את אותם
> ערכים — בלי טבלה נוספת, בלי מפתח זר, ובלי טעינה לזיכרון. טבלה נפרדת שמורה לנתונים
> שהמשתמשים עצמם יוצרים ומשנים.

### בבסיס הנתונים:
- [ ] שמרו את הערך כטקסט בעמודה של הישות עצמה (`NVARCHAR`) — לא כמפתח זר
- [ ] הוסיפו אילוץ `CHECK (<column> IN (N'...', N'...'))` עם כל הערכים החוקיים (ראו: `CK_WORKER_TITLE`)
- [ ] אין טבלת Lookup ואין `Get_all_XYZ` — הערכים מגיעים מה-enum שב-C#
...
- [ ] מלאו ComboBox מתוך `Enum.GetValues(typeof(XYZ))` — לא משאילתה ל-DB

### דוגמה:
```
DB:  Orders.status NVARCHAR(20)
     CONSTRAINT CK_ORDER_STATUS CHECK (status IN (N'ממתין לאישור', N'בטיפול', N'הושלם'))
C#:  enum Status { ממתין_לאישור, בטיפול, הושלם }
     StatusHelper.ToDisplayString(Status.ממתין_לאישור) → "ממתין לאישור"
```

### אימות:
- [ ] רשימת הערכים ב-`CHECK` זהה בדיוק לערכי ה-enum (כולל רווחים וניקוד)
- [ ] ה-Helper ממיר נכון לשני הכיוונים
- [ ] הכנסת ערך לא חוקי ל-DB נכשלת עם שגיאת CHECK

---

## דרג 1: טבלאות וישויות בסיסיות
...
## דרג 2: ישויות עם תלויות (Foreign Key)
...
#### בבסיס הנתונים:
- [ ] צרו טבלה כמו בדרג 1 (ראו: טבלת `Orders` ב-`scripts/create_database.sql`)
...
### בבסיס הנתונים:
- [ ] טבלת אב — שדות משותפים (כמו בדרג 1/2) (ראו: טבלת `Orders`)
...
## דרג 4: מחלקות קישור — Association Class (אם יש)
...
{
    // דרג 1: ישויות בסיסיות (בלי FK)
    // דרג 2: ישויות עם FK (אחרי מה שהן מפנות אליו)
    // דרג 3: ישויות נגזרות בירושה (אחרי טבלת האב)
    // דרג 4: מחלקות קישור (אחרונות — מפנות לשני צדדים)
}
```
