---
title: "חיבור Claude Code לבסיס הנתונים"
subtitle: "הגדרת שרת ה-MSSQL MCP, ופתרון תקלות"
...
## לפני שמאשימים את ה-MCP — בדקו את הרשת

**רלוונטי רק למסלול Azure.** ‏Azure SQL מאזין **רק בפורט 1433**, וברשת הסטודנטים של
האוניברסיטה הפורט הזה חסום ליציאה. התוצאה מבלבלת: ההתקנה מצליחה, השרת עולה, הכלים
מופיעים ברשימה — וכל קריאה ל-`execute_sql` נתקעת ומסתיימת בטיים-אאוט.

זה **לא** באג בחבילה, ולכן החלפת שרת ה-MCP לשרת אחר לא תפתור את זה.

בדיקה חד-משמעית, ב-PowerShell:

```powershell
Test-NetConnection <servername>.database.windows.net -Port 1433
```

- `TcpTestSucceeded : True` — הרשת בסדר. המשיכו לטבלת התקלות למטה.
- `TcpTestSucceeded : False` — **הרשת חוסמת אתכם.** הפתרון: חברו את המחשב לשיתוף
  האינטרנט (hotspot) של הטלפון והריצו שוב. מספיק שהמחשב שמריץ את Claude יהיה על ה-hotspot.

**סימן היכר מהיר:** אם גם **SSMS** לא מתחבר מאותה רשת, הבעיה היא הרשת — שניהם משתמשים
באותו פורט. אם SSMS דווקא כן מתחבר ורק ה-MCP נכשל, הבעיה היא בהגדרה, והטבלה למטה תעזור.

הבדיקה הזו מופיעה גם ב-`PREREQS` חלק ג2.5, כדי שתבצעו אותה **לפני** השיעור.
...
| Timeout בחיבור, או "Adaptive Server is unavailable" | **שתי סיבות אפשריות.** (א) הרשת שלכם חוסמת את פורט 1433 — בדקו עם `Test-NetConnection` לפי הסעיף שמעל הטבלה, וזו התשובה כמעט תמיד כשאתם בקמפוס. (ב) ה-Firewall של Azure חוסם אתכם: פורטל Azure ← ה**שרת** שלכם (לא בסיס הנתונים) ← Networking ← ודאו שכלל ה-Firewall‏ `0.0.0.0`–`255.255.255.255` קיים **ונשמר**. |
