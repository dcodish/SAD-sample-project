...
## ג2.5 — בדיקת פורט 1433 — **בצעו מהרשת של האוניברסיטה**

‏Azure SQL מקבל חיבורים **רק בפורט 1433**. ברשת הסטודנטים של האוניברסיטה הפורט הזה
חסום ליציאה — ולכן בסיס נתונים שמתחבר מצוין מהבית פשוט לא נפתח מהכיתה. לא מ-SSMS,
ולא מ-Claude Code.

**זו הייתה התקלה מספר אחת בשיעור הקודם**, והיא מתחזה לתקלת MCP: ההתקנה מצליחה,
השרת עולה, וכל קריאה נכשלת בטיים-אאוט. החלפת שרת ה-MCP לא תעזור — הבעיה היא הרשת.

**בדקו את זה מראש, כשאתם על הרשת שבה תשבו בשיעור.** ב-PowerShell:

```powershell
Test-NetConnection <servername>.database.windows.net -Port 1433
```

הסתכלו על השורה האחרונה:

- `TcpTestSucceeded : True` — הדרך פתוחה. אתם מסודרים.
- `TcpTestSucceeded : False` — הפורט חסום ברשת הזו. **אל תגיעו לשיעור בלי פתרון.**

> **הפתרון: נקודה חמה (hotspot) מהטלפון.** חברו את המחשב לשיתוף האינטרנט של הטלפון
> במקום לרשת האוניברסיטה, והריצו שוב את הבדיקה — כך הקבוצות שנתקעו פתרו את זה בשיעור.
> נפח הנתונים זניח: תעבורת ה-SQL היא טקסט, וגם Claude Code עצמו קל.
>
> מספיק שהמחשב **שמריץ את Claude** יהיה על ה-hotspot. אם אתם ממילא עובדים בשיטת
> "אחד מריץ, השאר בודקים" (מומלץ — ראו את סעיף המכסה), זה טלפון אחד לכל הקבוצה.
>
> **בדקו שהטלפון שלכם יודע לשתף אינטרנט לפני השיעור**, ושהבדיקה למעלה מחזירה `True`
> דרכו. בחלק מהמסלולים הסלולריים שיתוף האינטרנט כבוי כברירת מחדל.

**רשת קווית, VPN או רשת סגל** עשויות להתנהג אחרת מהרשת האלחוטית של הסטודנטים.
הבדיקה תקפה רק לרשת שממנה הרצתם אותה — הריצו אותה על הרשת שבה תעבדו בפועל.

**אם אין לכם hotspot זמין:** עדיין תוכלו לעבוד — ראו את מסלול ב׳ (SQL Server מקומי)
למטה. הוא לא דורש רשת בכלל, אבל אין בו בסיס נתונים משותף לקבוצה. זו החלטה שכדאי
לקבל **לפני** השיעור, לא באמצעו.
...
**התוצאה הצפויה:** פרטי גרסה שמציינים Azure SQL, ובסיס הנתונים שלכם ברשימה.

אם אתם מקבלים **timeout** בחיבור, יש שתי סיבות אפשריות, וכדאי להבדיל ביניהן:

- **מהבית:** כמעט תמיד כלל ה-Firewall ב-Azure לא נשמר — חזרו לשלב ג2 סעיף 6.
- **מהאוניברסיטה:** כמעט תמיד פורט 1433 חסום — ראו ג2.5. ‏SSMS ו-Claude Code משתמשים באותו פורט בדיוק, ולכן אם SSMS לא מתחבר מהקמפוס, גם ה-MCP לא יתחבר.
...
# חלק ד׳ — קובצי הניתוח של הקבוצה (PDF)

בשלב 2 של השיעור Claude ימיר את מסמכי חלק א׳ וחלק ב׳ שלכם ל-markdown. כמה דקות
הכנה כאן חוסכות שעה בכיתה.

## ד1 — ל-PDF שלכם חייבת להיות שכבת טקסט

יש שני סוגי PDF שנראים זהים לחלוטין על המסך:

| | מה זה | מה Claude עושה איתו |
|---|---|---|
| **PDF עם שכבת טקסט** | נוצר על ידי **ייצוא** מ-Word,‏ Google Docs או Visual Paradigm | קורא את הטקסט ישירות — מהיר וזול |
| **PDF שהוא תמונות** | סריקה, צילום מהטלפון, או "הדפסה" לתמונה | חייב **להסתכל על כל עמוד כתמונה**, אחד-אחד |

הסוג השני הוא שהפך את שלב 2 לשעה וחצי אצל אחת הקבוצות בשיעור הקודם, ובדרך שרף גם
את מכסת הטוקנים שלהם.

**הבדיקה, עשר שניות:** פתחו את ה-PDF, נסו **לסמן שורת טקסט עם העכבר**, או חפשו בו מילה
ב-`Ctrl+F`. אם אי אפשר לסמן ואי אפשר לחפש — זה PDF של תמונות, ויש לייצא אותו מחדש.

## ד2 — איך מייצאים נכון

- **Word:** ‏File ← **Save As** (או Export ← Create PDF) ← בחרו `PDF` ברשימת סוגי הקובץ.
  **אל** תשתמשו ב-`Print` ← "Microsoft Print to PDF" אם אפשר להימנע — הוא לפעמים מייצר עמודי תמונה.
- **Google Docs:** ‏File ← Download ← **PDF Document**.
- **Visual Paradigm:** ‏Project ← Export ← **PDF**.
- **סריקה של תדפיס:** לא מתאים. מצאו את הקובץ המקורי.

## ד3 — הביאו גם את קובץ המקור

שימו ב-`docs/`, לצד ה-PDF, גם את **קובץ ה-Word המקורי** (`.docx`) של חלק א׳ וחלק ב׳.
‏Claude קורא `.docx` ישירות ובזול, וזו רשת הביטחון הטובה ביותר אם משהו ב-PDF יוצא עקום.

**הדיאגרמות הן חריג ונשארות תמונות** — זה בסדר גמור ומצופה. הן ממילא מעטות
(דיאגרמת מחלקות, מצבים, רצף, UC), ו-Claude יפתח רק את העמודים שלהן. מה שחייב להיות
טקסט אמיתי הוא הגוף של המסמך: הדרישות, הטבלאות, מפרטי ה-UC והראיונות.

בתחילת שלב 2 בשיעור נריץ בדיקה אוטומטית שמאשרת את כל זה לפני שמתחילים לחלץ.
...
- [ ] קובצי ה-PDF של חלק א׳ וחלק ב׳ של הקבוצה נגישים לכם — **ואפשר לסמן בהם טקסט עם העכבר** (חלק ד׳)
- [ ] קובצי ה-Word המקוריים (`.docx`) של חלק א׳ וחלק ב׳ נמצאים איתכם כגיבוי
- [ ] ☁️ מסלול Azure: ‏`Test-NetConnection <servername>.database.windows.net -Port 1433` מחזיר `True` **מהרשת שבה תשבו בשיעור** (חלק ג2.5)
- [ ] ☁️ מסלול Azure: אם התשובה הייתה `False` — שיתוף האינטרנט בטלפון עובד ונבדק
...
| ‏Azure: הכול עובד מהבית ושום דבר לא מתחבר מהאוניברסיטה | פורט 1433 חסום ברשת הסטודנטים | זו לא תקלת MCP ולא תקלת Firewall. חברו את המחשב ל-hotspot מהטלפון — ראו חלק ג2.5. אמתו עם `Test-NetConnection <servername>.database.windows.net -Port 1433` |
| שלב 2 (חילוץ ה-PDF) נמשך שעה ויותר | ל-PDF אין שכבת טקסט — Claude מסתכל על כל עמוד כתמונה | ייצאו את ה-PDF מחדש מקובץ ה-Word המקורי, או תנו ל-Claude את ה-`.docx` עצמו. ראו חלק ד׳ |
